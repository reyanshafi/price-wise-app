          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            ...
            <div className="h-64 flex items-end justify-between space-x-2">
              {[45, 67, 89, 123, 156, 134, 178, 165, 142, 198, 234, 267].map((height, index) => (
                <div key={index} className="flex-1 bg-gradient-to-t from-blue-500 to-purple-500 rounded-t-lg relative">
                  <div 
                    className="bg-gradient-to-t from-blue-500 to-purple-500 rounded-t-lg transition-all duration-1000"
                    style={{ height: `${height}px` }}
                  ></div>
                  <div className="absolute -bottom-6 left-0 right-0 text-center text-xs text-indigo-200">
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-white/5 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-indigo-200 text-sm">{activity.product}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-green-400 font-semibold">{activity.amount}</p>
                  <p className="text-indigo-200 text-sm">{activity.time}</p>
                </div>
              </div>
            ))}
          </div>
